...
const CreateVideoButton = () => {
    ...
    const { btnCreateVideo, loadCreateState } = useContext(VideoThreadContext);

  return (
    <div onClick={btnCreateVideo} className='group-button-modal non-select' style={{float: 'left'}}>
        {loadCreateState == false ? 
            <span> 
