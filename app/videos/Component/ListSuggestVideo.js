import { worker_image_domain } from '@/app/env_video'
...
const ListSuggestVideo = ({data}) => {
  ...
  const [myFolder, setMyFolder] = useState("Miwabox Video")

  useEffect(() => {
    if(folder) setMyFolder(capitalizeWords(folder));
  }, [folder]); 
        ...
        <div id='my-suggested-video-container'>
            {data?.map((item, index) => (
              <Link href={'/?id='+currentIndex+'&folder='+folder+'&vid='+item.index}>
                <div key={String(item.index) + String(index)} className='item-suggested-video'>
                    ...
                    <img src= {worker_image_domain[Math.floor(Math.random() * worker_image_domain.length)]  + '?id=' + item?.thumbId}  alt={myFolder}/>
                </div>
              </Link>
            ))}
