...
import { url_download_video_page, url_video_domain } from '@/app/env_video'
...
const MyVideoPage = ({myData, sugData, videoSrc}) => {
  ...
  const {deleteVideoData} = useContext(VideoThreadContext);
  const {vid} = useContext(VideoPageContext);
  ...
  const deleteMyVideoFile = () => {
    const url = `${url_video_domain}file/get/${vid}`;
    const options = {
      method: 'GET',
      headers: {
    ...
    fetch(url, options)
     ...
     .then(async data => {
        if (data) {
          if (confirm("You want delete this video on data?!") == true) {
          try {
            await deleteVideoData(data.thumbId, vid) 
            alert('Video data deleted successfully.')
          } catch (error) {
            alert(error)
          }
          }
        } else {
          alert('Couldn\'t delete a null object.')
        }
      })
     .catch(error => {
  ...
  return (
    <>
        ...
        <div id='info-video-page-container'>     
            {myData?.title ? <div id='my-title-video-page'>{myData?.title}</div> : ""}
            <div id='my-info-video-page-container' className='non-select'>
                ...
                <span onClick={() => {window.open(url_download_video_page+'?ciphertext='+myData?.directUrl, '_blank');}}><svg viewBox="0 0 24 24" fill="none"><path d="M12.5 4V17M12.5 17L7 12.2105M12.5 17L18 12.2105" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path><path d="M6 21H19" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path></svg>Download</span>
                <span onClick={ () => { 
